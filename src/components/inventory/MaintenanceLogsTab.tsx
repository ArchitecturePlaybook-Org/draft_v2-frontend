...
import { Wrench, Plus, RefreshCw, AlertTriangle, Zap, Calendar, DollarSign, BarChart3, List } from "lucide-react";
...
export function MaintenanceLogsTab() {
  ...
  const costByTypeData = useMemo(() => {
    const map = new Map();
    logs.forEach(l => {
      const cost = Number(l.service_cost || 0);
      if (cost > 0) {
        map.set(l.service_type, (map.get(l.service_type) || 0) + cost);
      }
    });
    return Array.from(map.entries()).map(([k, v]) => ({
      name: SERVICE_TYPE_CONFIG[k as ServiceType]?.label || k,
      value: v,
      color: SERVICE_TYPE_CONFIG[k as ServiceType]?.color.replace("text-", "bg-") || "bg-zinc-500",
      hex: k === "SCHEDULED" ? "#60a5fa" : k === "BREAKDOWN" ? "#f87171" : k === "CALIBRATION" ? "#fbbf24" : k === "INSPECTION" ? "#2dd4bf" : "#c084fc"
    }));
  }, [logs]);

  const downtimeByEqData = useMemo(() => {
    const map = new Map();
    logs.forEach(l => {
      const dt = Number(l.downtime_days || 0);
      if (dt > 0) {
        map.set(l.equipment_name, (map.get(l.equipment_name) || 0) + dt);
      }
    });
    return Array.from(map.entries())
      .map(([name, downtime]) => ({ name, downtime }))
  ...
  const monthlyCostData = useMemo(() => {
    ...
    const sorted = Array.from(map.entries()).map(([month, cost]) => ({ month, cost }));
    return sorted; // In a real app, sort by actual chronological order instead of string/map insertion order
  }, [logs]);
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto text-zinc-100 font-sans animate-in fade-in duration-200">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center bg-zinc-900 border border-zinc-700 rounded-xl p-1 mr-2">
            <button
              onClick={() => setViewMode("HISTORY")}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all ${viewMode === "HISTORY" ? "bg-zinc-800 text-white shadow-sm" : "text-zinc-500 hover:text-zinc-300"}`}
            >
              <List className="w-3.5 h-3.5" /> History
            </button>
            <button
              onClick={() => setViewMode("ANALYTICS")}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all ${viewMode === "ANALYTICS" ? "bg-amber-500/10 text-amber-500 border border-amber-500/20 shadow-sm" : "text-zinc-500 hover:text-zinc-300"}`}
            >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {[
          ...
          {label:"Avg Cost / Svc", value:`₹${logs.length ? Math.round(totalCost/logs.length).toLocaleString("en-IN") : 0}`, color:"text-teal-400"},
          {label:"Total Downtime", value:`${totalDowntime}d`,         color:"text-orange-400"},
        ].map(s=>(
          <div key={s.label} className="p-4 rounded-xl bg-zinc-950 border border-zinc-800 flex flex-col justify-between">
            <p className="text-[10px] uppercase font-bold text-zinc-500">{s.label}</p>
            <p className={`text-lg sm:text-xl font-black mt-1 ${s.color}`}>{s.value}</p>
          </div>
        ))}
      </div>
      ...
      {viewMode === "ANALYTICS" ? (
        <div className="space-y-4 animate-in fade-in duration-200">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-5 shadow-sm">
              <h3 className="text-xs font-bold text-zinc-300 uppercase tracking-wider mb-4">Service Cost Breakdown</h3>
              <div className="h-64">
                {costByTypeData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={costByTypeData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5}>
                  ...
                  <div className="flex items-center justify-center h-full text-xs text-zinc-500">No cost data available</div>
                )}
              </div>
            </div>

            {/* Monthly Trend Area Chart */}
            <div className="md:col-span-2 bg-zinc-950 border border-zinc-800 rounded-2xl p-5 shadow-sm">
              <h3 className="text-xs font-bold text-zinc-300 uppercase tracking-wider mb-4">Monthly Maintenance Costs</h3>
              <div className="h-64">
                {monthlyCostData.length > 0 ? (
                  ...
                  <div className="flex items-center justify-center h-full text-xs text-zinc-500">No date trend data available</div>
                )}
              </div>
            </div>

            {/* Downtime Bar Chart */}
            <div className="md:col-span-3 bg-zinc-950 border border-zinc-800 rounded-2xl p-5 shadow-sm">
              <h3 className="text-xs font-bold text-zinc-300 uppercase tracking-wider mb-4">Equipment Downtime Leaderboard (Days)</h3>
              <div className="h-64">
                {downtimeByEqData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={downtimeByEqData} margin={{ top: 10, right: 10, left: -20, bottom: 20 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#27272a" vertical={false} />
                      <XAxis dataKey="name" stroke="#71717a" fontSize={10} tickLine={false} axisLine={false} angle={-15} textAnchor="end" />
                      <YAxis stroke="#71717a" fontSize={10} tickLine={false} axisLine={false} />
                      <Tooltip cursor={{ fill: '#27272a' }} contentStyle={{ backgroundColor: '#18181b', borderColor: '#27272a', fontSize: '12px' }} />
                      <Bar dataKey="downtime" fill="#fb923c" radius={[4, 4, 0, 0]} barSize={40} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="flex items-center justify-center h-full text-xs text-zinc-500">No downtime data available</div>
                )}
              </div>
            </div>
        ...
        <div className="border border-zinc-800 rounded-xl overflow-hidden bg-zinc-950 shadow-md animate-in fade-in duration-200">
