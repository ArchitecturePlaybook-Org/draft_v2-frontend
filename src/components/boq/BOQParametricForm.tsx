...
import { BOQParameters, TYPOLOGY_OPTIONS, DEFAULT_PARAMS } from "@/domains/boq/types";
  ...
  return (
    <div className="boq-form">
      ...
      <div className="boq-section">
        <div className="boq-section-title">🏗️ Building Typology</div>
        <Field label="Select Structure Type">
          <select
            className="boq-select"
            value={params.typology}
            onChange={(e) => set("typology", e.target.value as BOQParameters["typology"])}
          >
        ...
        <button className="boq-reset-btn" onClick={() => onChange(DEFAULT_PARAMS)}>Reset to defaults</button>
      </div>

      {/* Outer Envelope */}
      <div className="boq-section">
        <div className="boq-section-title">📐 Outer Building Envelope</div>
        <div className="boq-2col">
          <Field label="Length (m)">
            <NumInput value={params.outer_length} min={3} max={200} step={0.5} onChange={(v) => set("outer_length", v)} />
          </Field>
          <Field label="Width (m)">
            <NumInput value={params.outer_width} min={3} max={200} step={0.5} onChange={(v) => set("outer_width", v)} />
          </Field>
        </div>
        <div className="boq-2col">
          <Field label="Floor Height (m)" hint="Floor-to-floor height">
            <NumInput value={params.floor_height} min={2.4} max={6} step={0.1} onChange={(v) => set("floor_height", v)} />
          </Field>
          <Field label="Number of Floors">
            <NumInput value={params.num_floors} min={1} max={20} step={1} onChange={(v) => set("num_floors", Math.floor(v))} />
          </Field>
        </div>
      ...
      {/* Internal Partition Walls — KEY DIFFERENTIATOR */}
      <div className="boq-section">
        ...
        <Field
          ...
          hint={`Auto-suggested: ${Math.round((2 * (params.outer_length + params.outer_width)) * 1.15)} m for this building (1.15× perimeter rule-of-thumb)`}
        >
          <SliderInput
            value={params.inner_wall_length}
            min={0}
            max={Math.round(params.outer_length * params.outer_width * 0.4)}
            step={0.5}
            unit="m"
        ...
        <Field label="Inner Doors (count)" hint="Internal room-to-room doors">
          <NumInput value={params.inner_door_count} min={0} max={30} step={1} onChange={(v) => set("inner_door_count", Math.floor(v))} />
        </Field>
        <Field label="Inner Door Avg Size (m²)" hint="Typical: 0.9m × 2.1m = 1.89m²">
      ...
      {/* Outer Openings */}
      <div className="boq-section">
        <div className="boq-section-title">🚪 External Openings</div>
        <div className="boq-2col">
          <Field label="Outer Doors (count)">
            <NumInput value={params.outer_door_count} min={0} max={10} step={1} onChange={(v) => set("outer_door_count", Math.floor(v))} />
          </Field>
          <Field label="Door Avg Size (m²)" hint="0.9×2.0 = 1.80 m²">
            <NumInput value={params.outer_door_size_m2} min={0.8} max={4} step={0.1} onChange={(v) => set("outer_door_size_m2", v)} />
          </Field>
        </div>
        <div className="boq-2col">
          <Field label="Outer Windows (count)">
            <NumInput value={params.outer_window_count} min={0} max={40} step={1} onChange={(v) => set("outer_window_count", Math.floor(v))} />
          </Field>
          <Field label="Window Avg Size (m²)" hint="1.2×1.2 = 1.44 m²">
            <NumInput value={params.outer_window_size_m2} min={0.3} max={6} step={0.1} onChange={(v) => set("outer_window_size_m2", v)} />
          </Field>
        </div>
      </div>
      ...
      {/* Foundation & Site */}
      <div className="boq-section">
        <div className="boq-section-title">⛏️ Foundation & Site</div>
        ...
        <Field label="Excavation Depth (m)" hint="Foundation depth below GL">
          <SliderInput value={params.excavation_depth} min={0.6} max={3.0} step={0.1} unit="m" onChange={(v) => set("excavation_depth", v)} />
        </Field>
      </div>
    </div>
  );
