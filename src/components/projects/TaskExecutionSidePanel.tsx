...
export const TaskExecutionSidePanel: React.FC<TaskExecutionSidePanelProps> = ({
  ...
  const [reallocateAssetTarget, setReallocateAssetTarget] = useState<{ asset: ProjectAsset; existingLink: any } | null>(null);
                      ...
                      ((a.category === "3d_model" || a.category === "sh3d") && !linked3dModel)
                        ...
                        <div className="bg-surface-100 border-surface-200 rounded-2xl border border-surface-200 p-10 text-center shadow-sm space-y-6 shrink-0">
                          <div>
                            ...
                            {(() => {
                              const getAssetAllocatedTask = (asset: ProjectAsset) => {
                                if (asset.category !== "3d_model" && asset.category !== "sh3d") return null;
                                return asset.linked_tasks?.find(t => t.uid !== task.uid);
                              };

                              const filteredList = projectAssets.filter(a => {
                                if (searchTerm.trim() && !a.title.toLowerCase().includes(searchTerm.toLowerCase())) {
                                  return false;
                                }
                                if (a.category === "2d_plan" && linked2dPlanUids.has(a.canonical_uid)) {
                                  return false;
                                }
                                return true;
                              });

                              const selectedAsset = projectAssets.find(a => a.canonical_uid === selectedAssetToLink);

                              return (
                                <div className="max-w-sm mx-auto p-5 bg-surface-50 rounded-2xl border border-surface-200 text-left space-y-4">
                                  <label className="block text-[10px] font-black text-surface-400 uppercase tracking-widest">Select Asset to Link</label>
                                  
                                  <div className="relative" ref={dropdownRef}>
                                    <button
                                      type="button"
                                      onClick={() => setIsOpenDropdown(!isOpenDropdown)}
                                      className="w-full h-11 bg-surface-50 border border-surface-200 hover:border-accent hover:bg-surface-100/50 px-4 rounded-xl flex items-center justify-between text-left font-bold text-xs text-primary transition-all shadow-xs"
                                    >
                                      <span className="truncate">
                                        {selectedAsset 
                                          ? `${selectedAsset.category === "2d_plan" ? "📐" : "🏛️"} ${selectedAsset.title}` 
                                          : "Select Asset..."}
                                      </span>
                                      <ChevronDown className={`w-4 h-4 text-surface-400 transition-transform duration-200 ${isOpenDropdown ? "rotate-180" : ""}`} />
                                    </button>

                                    {isOpenDropdown && (
                                      <div className="absolute left-0 right-0 mt-2 z-[100] bg-surface-card border border-surface-200 rounded-2xl shadow-xl p-3 flex flex-col gap-2 max-h-80 min-w-[280px]">
                                        <div className="relative">
                                          ...
                                          <input
                                            ...
                                            className="w-full bg-surface-50 border border-surface-200 rounded-xl pl-9 pr-4 py-2 text-xs text-primary outline-none focus:border-accent transition-colors"
                                          />
                                        </div>
                                        <div className="flex-1 overflow-y-auto max-h-48 space-y-1 custom-scrollbar">
                                          {filteredList.length === 0 ? (
                                            <p className="text-[10px] text-surface-400 text-center py-4 font-bold uppercase tracking-wider">No assets found</p>
                                          ) : (
                                            filteredList.map(a => {
                                              const is2d = a.category === "2d_plan";
                                              const is3d = a.category === "3d_model" || a.category === "sh3d";
                                              
                                              const isLinkedToThisTask = is2d
                                                ? linked2dPlanUids.has(a.canonical_uid)
                                                : (linked3dModel?.canonical_uid === a.canonical_uid);

                                              const allocatedTask = getAssetAllocatedTask(a);
                                              const isLockedByOtherTask = !!allocatedTask;
                                              const isDisabledDueToExisting3D = is3d && !isLinkedToThisTask && !!linked3dModel;

                                              const isSelectable = !isLinkedToThisTask && !isDisabledDueToExisting3D;
                                              ...
                                              return (
                                                <button
                                                  ...
                                                  onClick={() => {
                                                    if (isLockedByOtherTask) {
                                                      setReallocateAssetTarget({ asset: a, existingLink: a.linked_tasks?.[0] });
                                                      setIsOpenDropdown(false);
                                                      setSearchTerm("");
                                                      return;
                                                    }
                                                    setSelectedAssetToLink(a.canonical_uid);
                                                    setIsOpenDropdown(false);
                                                    setSearchTerm("");
                                                  }}
                                                  className={`w-full text-left p-2.5 rounded-xl flex items-center justify-between transition-colors ${
                                                    isSelectable
                                                      ? "hover:bg-surface-200/60 text-primary"
                                                      : "opacity-60 bg-surface-100/50 text-surface-400 cursor-not-allowed"
                                                  }`}
                                                >
                                                  <div className="min-w-0 flex-1 pr-2">
                                                    <div className="flex items-center gap-2">
                                                      <span className="text-sm shrink-0">{is3d ? "🏛️" : "📐"}</span>
                                                      <p className="text-xs font-bold truncate text-primary">{a.title}</p>
                                                    </div>
                                                    {isLockedByOtherTask && (
                                                      <p className="text-[8px] font-black text-indigo-500 uppercase tracking-wider mt-1 flex items-center gap-1">
                                                        <span>🔄</span> Click to Re-allocate — Allocated to: "{allocatedTask.title}"
                                                      </p>
                                                    )}
                                                    {isDisabledDueToExisting3D && (
                                                      <p className="text-[8px] font-black text-amber-500 uppercase tracking-wider mt-1 flex items-center gap-1">
                                                        <span>⚠️</span> Locked — Current task already has a 3D model linked
                                                      </p>
                                                    )}
                                                    {isLinkedToThisTask && (
                                                      <p className="text-[8px] font-black text-emerald-500 uppercase tracking-wider mt-1 flex items-center gap-1">
                                                        <span>✓</span> Already linked to this task
                                                      </p>
                                                    )}
                                                  </div>
                                                  {isLinkedToThisTask && (
                                                    <span className="text-[8px] font-black uppercase tracking-widest text-emerald-600 bg-emerald-500/10 px-2 py-0.5 rounded-lg border border-emerald-500/20 shrink-0">
                                                      Linked
                                                    </span>
                                                  )}
                                                  {isLockedByOtherTask && (
                                                    <span className="text-[8px] font-black uppercase tracking-widest text-indigo-600 bg-indigo-500/10 px-2 py-0.5 rounded-lg border border-indigo-500/20 shrink-0">
                                                      Re-allocate
                                                    </span>
                                                  )}
                                                </button>
                                  ...
                                  <button
                                    ...
                                    className="w-full h-11 bg-accent text-background font-black text-[10px] uppercase tracking-[0.2em] rounded-xl hover:opacity-90 transition-all disabled:opacity-40"
                                  >
          ...
          <div className="bg-surface-card dark:bg-[#0b0f19] border border-surface-200 dark:border-white/10 w-full max-w-sm rounded-2xl shadow-xl overflow-hidden p-5 flex flex-col gap-4 text-center scale-in-center">
            ...
            <div>
              ...
              <p className="text-[11px] font-medium text-surface-500 dark:text-white/60 mt-2 leading-relaxed">
                This 3D model is currently linked to <strong className="text-primary dark:text-accent">"{reallocateAssetTarget.existingLink.title}"</strong>.<br />
                Would you like to unlink it from "{reallocateAssetTarget.existingLink.title}" and link it to this task instead?
              </p>
            </div>
            <div className="flex gap-2.5 mt-2">
              <button
                type="button"
                onClick={() => setReallocateAssetTarget(null)}
              ...
              <button
                type="button"
                onClick={async () => {
                  const oldLinkId = reallocateAssetTarget.existingLink.id;
                  setReallocateAssetTarget(null);
                  setIsLinking(true);
                  try {
                    // Step 1: Unlink from old task link
                    await projectsApi.unlinkAssetFromTask(oldLinkId);
                    // Step 2: Link to new task
                    await projectsApi.linkAssetToTask(task.uid, reallocateAssetTarget.asset.canonical_uid);
                    
                    // Refresh project details and task data
                    await refreshTask();
                    const { useProjectStore } = await import("@/store/project-store");
                    const freshData = await projectsApi.getProjectDetails(projectUid);
                    useProjectStore.setState({ project: freshData });

                    toast.success(`Successfully re-allocated model to this task`);
                  } catch (err) {
