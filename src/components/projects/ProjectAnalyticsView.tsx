...
export function ProjectAnalyticsView({ projectUid }: ProjectAnalyticsViewProps) {
  ...
  if (!data) return <div className="py-20 text-center text-surface-500">Failed to load data.</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-2xl border border-surface-200 shadow-sm flex flex-col items-center justify-center min-h-[200px]">
        ...
        <div className="mt-4 text-xs font-bold text-surface-500">
          {data.completed_tasks} of {data.total_tasks} tasks done
        </div>
      </div>

      {/* Task Status Distribution */}
      <div className="bg-white p-6 rounded-2xl border border-surface-200 shadow-sm min-h-[200px]">
        ...
        <div className="space-y-4">
          {Object.entries(data.tasks_by_status).map(([status, count]: [string, any]) => (
            <div key={status} className="flex items-center gap-3">
              <div className="w-16 text-[10px] font-bold text-surface-500">{status}</div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-surface-200 shadow-sm flex flex-col justify-center min-h-[200px]">
