...
export const DiaryEntryDetail: React.FC<DiaryEntryDetailProps> = ({ entry, projectId, onUpdate, onClose }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-surface-200 shadow-xl flex flex-col h-full min-h-[600px]">
      {/* Header */}
      <div className="sticky top-0 z-10 p-5 border-b border-surface-200 bg-white rounded-t-2xl flex justify-between items-center shrink-0 shadow-sm">
        <div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-surface-50">
        ...
        <div className="bg-white border border-surface-200 rounded-xl overflow-hidden shadow-sm">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              {!expandedSections.metadata && (
                <span className="text-sm font-medium text-surface-500 ml-4">
          ...
          {expandedSections.metadata && (
            <div className="p-5 border-t border-surface-100 bg-white space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-bold text-surface-500 uppercase mb-2">Sky Conditions</label>
                    ...
                    {weatherOptions.map(opt => (
                      <button
                        ...
                        className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm font-medium transition-colors ${
                          entry.sky_conditions === opt.value ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white hover:bg-surface-50 text-surface-700 disabled:opacity-50'
                        }`}
                      >
                        {opt.icon} {opt.label}
                      </button>
                ...
                <div>
                  <label className="block text-xs font-bold text-surface-500 uppercase mb-2">Site Conditions</label>
                  <div className="flex flex-wrap gap-2">
                    {siteConditionOptions.map(opt => (
                      <button
                        ...
                        className={`px-3 py-2 rounded-lg border text-sm font-medium transition-colors ${
                          entry.site_conditions === opt ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white hover:bg-surface-50 text-surface-700 disabled:opacity-50'
                        }`}
                      >
                        {opt}
                      </button>
                ...
                <div className="flex gap-4">
                  <div className="flex-1">
                    <label className="block text-xs font-bold text-surface-500 uppercase mb-2">Temp High (°C)</label>
                  ...
                  <div className="flex-1">
                    <label className="block text-xs font-bold text-surface-500 uppercase mb-2">Temp Low (°C)</label>
        ...
        <div className="bg-white border border-surface-200 rounded-xl overflow-hidden shadow-sm">
          ...
          {expandedSections.progress && (
            <div className="p-5 border-t border-surface-100 bg-white space-y-4">
                  ...
                  {entry.activities?.map((act: any) => (
                    <div key={act.id} className="p-4 border border-surface-200 rounded-xl bg-surface-50">
                      ...
                      {(act.progress_percent || act.hours) && (
                        <div className="mt-3 text-xs text-surface-500 font-bold flex gap-4">
            ...
            <div className="p-5 border-t border-surface-100 bg-white space-y-4">
                ...
                {entry.labor_entries?.map((l: any) => (
                  <div key={l.id} className="p-4 bg-surface-50 border border-surface-200 rounded-xl flex justify-between items-center">
                    ...
                    <div className="text-right">
                      <p className="font-bold text-orange-600 text-lg">{l.headcount} <span className="text-sm font-normal text-surface-500">workers</span></p>
                      <p className="text-xs font-bold text-surface-500 uppercase">{l.total_hours} hrs</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {!isLocked && (
                <div className="p-4 border-2 border-dashed border-surface-200 rounded-xl bg-surface-50/50">
                  <h4 className="text-xs font-bold text-surface-500 uppercase mb-3 flex items-center gap-1"><Plus className="w-3 h-3" /> Log New Crew</h4>
        ...
        <div className="bg-white border border-surface-200 rounded-xl overflow-hidden shadow-sm">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              {!expandedSections.materials && (
                <span className="text-sm font-bold text-emerald-600 ml-4 bg-emerald-50 px-2 py-0.5 rounded-md">
          ...
          {expandedSections.materials && (
            <div className="p-5 border-t border-surface-100 bg-white space-y-4">
                ...
                {entry.material_entries?.map((m: any) => (
                  <div key={m.id} className="p-4 bg-surface-50 border border-surface-200 rounded-xl flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-xs font-medium text-surface-500 mt-1">Supplier: {m.supplier} • Ticket: {m.ticket_number}</p>
                    </div>
                    <div className="text-right flex flex-col items-end">
                      <p className="font-bold text-emerald-600 text-lg bg-emerald-50 px-2 py-0.5 rounded">{m.quantity} <span className="text-sm font-normal text-emerald-700">{m.unit}</span></p>
              ...
              {!isLocked && (
                <div className="p-4 border-2 border-dashed border-surface-200 rounded-xl bg-surface-50/50">
                   <h4 className="text-xs font-bold text-surface-500 uppercase mb-3 flex items-center gap-1"><Plus className="w-3 h-3" /> Log Delivery</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="bg-white border border-surface-200 rounded-xl overflow-hidden shadow-sm">
          ...
          {expandedSections.equipment && (
            <div className="p-5 border-t border-surface-100 bg-white space-y-4">
                ...
                {entry.equipment_entries?.map((eq: any) => (
                  <div key={eq.id} className="p-4 bg-surface-50 border border-surface-200 rounded-xl flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-xs font-medium text-surface-500 mt-1 uppercase tracking-wider">{eq.hours_operated}h operated • {eq.hours_idle}h idle</p>
                    </div>
                    <div>
              ...
              {!isLocked && (
                <div className="p-4 border-2 border-dashed border-surface-200 rounded-xl bg-surface-50/50">
                  <h4 className="text-xs font-bold text-surface-500 uppercase mb-3 flex items-center gap-1"><Plus className="w-3 h-3" /> Log Equipment</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="bg-white border border-surface-200 rounded-xl overflow-hidden shadow-sm">
          <div 
            className="p-4 flex justify-between items-center cursor-pointer hover:bg-red-50 select-none transition-colors"
          ...
          >
            <div className="flex items-center gap-3">
              ...
              {!expandedSections.delays && entry.delay_entries?.length > 0 && (
                <span className="text-sm font-bold text-red-600 ml-4 bg-red-50 px-2 py-0.5 rounded-md border border-red-200">
            ...
            <div className="p-5 border-t border-surface-100 bg-white space-y-4">
              <div className="space-y-3">
                {entry.delay_entries?.map((d: any) => (
                  <div key={d.id} className="p-4 bg-red-50 border border-red-200 rounded-xl">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="text-xs font-bold bg-white text-red-700 px-2 py-1 rounded-md border border-red-200">{d.duration_hours} hrs</span>
                    </div>
                    <p className="text-sm text-red-900 font-medium bg-white/50 p-2 rounded-lg border border-red-100">Impact: {d.impacted_path}</p>
                  </div>
                ))}
              </div>
              ...
              {!isLocked && (
                <div className="p-4 border-2 border-dashed border-red-200 rounded-xl bg-red-50/30">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <select className="h-10 px-3 rounded-lg border border-red-200 text-sm font-medium focus:ring-red-500" value={newDelay.delay_type} onChange={e => setNewDelay({...newDelay, delay_type: e.target.value})}>
                    ...
                    <input type="number" placeholder="Duration (hrs)" className="h-10 px-3 rounded-lg border border-red-200 text-sm focus:ring-red-500" value={newDelay.duration_hours} onChange={e => setNewDelay({...newDelay, duration_hours: e.target.value})}/>
                    <input type="text" placeholder="Impacted Path / Tasks" className="h-10 px-3 rounded-lg border border-red-200 col-span-1 md:col-span-2 text-sm focus:ring-red-500" value={newDelay.impacted_path} onChange={e => setNewDelay({...newDelay, impacted_path: e.target.value})}/>
        ...
        <div className="bg-white border border-surface-200 rounded-xl overflow-hidden shadow-sm">
          ...
          {expandedSections.safety && (
            <div className="p-5 border-t border-surface-100 bg-white space-y-5">
              ...
              {safety.incident_reported && (
                <div>
                  ...
                  <textarea disabled={isLocked} className="w-full p-3 rounded-xl border border-red-200 bg-red-50 focus:bg-white focus:ring-red-500 text-sm" rows={3} placeholder="Describe the incident..." value={safety.incident_description} onChange={e => setSafety({...safety, incident_description: e.target.value})}></textarea>
                </div>
              )}
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-surface-500 uppercase mb-2">Inspections Conducted</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-surface-500 uppercase mb-2">Visitors On Site</label>
